...
class Repository {
        ...
        if let items = getItems() {
            for item in items {
                if let image = getImageFromXcAssets(item: item), let fileName = getImageNameFromXcAssets(item: item) {
                    ImageManager.storeImage(image: image, with: fileName, to: .application)
                }
            }
        }
    }
    ...
    func getImageFromFilesystem(item: Item) -> UIImage? {
        ...
        if let firstImage = item.images?.allObjects.first as? Image {
            ...
            print(ImageManager.getPath(for: fileName, in: .application) ?? "no path found for \(firstImage.item.label ?? "unknown item")")
            return ImageManager.getImage(from: fileName, in: .application)
        }
